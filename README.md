...
- 多部裝置可以拉取共享訂單
- 產品、價錢及優惠設定應只由指定管理人操作；目前程式未真正鎖定產品管理機
...
```text
catstaste_stable_v21_3_1_qa_fix/
├── index.html              # 主 App：落單、訂單、後台、產品管理、同步邏輯
├── Code.gs                 # Google Apps Script 後端
├── service-worker.js       # PWA 離線快取，v21.3.1 cache
...
- App 畫面加入版本顯示：`v21.3.1 QA 修正版`
- `service-worker.js` cache 更新至 `catstaste-order-v21-3-1-qa-fix`，避免手機繼續食舊版 cache


### v21.3 優惠門檻修正

- 修正滿 $150 九折計算邏輯：
  - 滿 $150 門檻改用 `thresholdable = TRUE` 的商品小計計算。
  - 預設所有產品都會計入滿 $150 / 贈品門檻。
  - 滿額後，九折只會扣 `discountable = TRUE` 的單件產品。
  - 不參與九折的產品仍可幫全單達到 $150 門檻。
- 新增產品欄位 `thresholdable`。
- 產品管理內可設定每款產品是否「計滿額」。
- `service-worker.js` cache 更新至 `catstaste-order-v21-3-1-qa-fix`。

例子：一箱主食罐 $220（不九折但計滿額）+ 單件產品 $20（可九折）= 滿額 $240，九折只扣 $20，即優惠 -$2，應收 $238。

### v21 / v21.3.1 已完成內容

- 新增 **可九折 / 計滿額** 兩組產品設定
  - `discountable`：是否參與九折
  - `thresholdable`：是否計入滿 $150 / 贈品門檻
- 修正優惠邏輯
  - 滿 $150 用「計滿額商品」計算
  - 九折只扣「可九折單件產品」
  - 原箱價可以計滿額，但原箱本身不九折
- 新增 **產品最後更新時間** 顯示
- 訂單 ID 使用日期時間加隨機碼，降低撞號風險
- 訂單建立後會清楚顯示：
  - 已保存在本機
  - 雲端同步中 / 未同步 / 已同步
...
> 重要限制：目前程式碼未真正實作「產品管理機鎖定」、「deviceId/deviceLabel」或 `itemType` 欄位。現場要靠營運規則確保只由一部機改產品。
...
1. 指定一部機由管理人使用。
2. 只有呢部機可以進入後台新增 / 編輯產品。
3. 其他同事機只負責落單、查單、填快遞。
4. 目前程式未有真正鎖定功能，所以要靠現場規則執行。
...
注意：目前這版未有 `itemType` 欄位，袋類要靠 `discountable` / `thresholdable` 設定處理。
...
訂單同步有 `LockService`，多人同時落單時會排隊寫入 Google Sheet。這版訂單 ID 包含日期時間及隨機碼，撞號風險比舊版低。

### 產品

目前程式未真正鎖定產品管理機，建議營運上只由一部機改產品。產品同步會用 `id + updatedAt` merge，減少不同產品互相覆蓋。
