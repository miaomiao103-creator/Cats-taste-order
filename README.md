...
```text
catstaste_stable_v21_1_notes/
├── index.html              # 主 App：落單、訂單、後台、產品管理、同步邏輯
├── Code.gs                 # Google Apps Script 後端
├── service-worker.js       # PWA 離線快取，v21.1 cache
...
- App 畫面加入版本顯示：`v21.1 備註穩定版`
- `service-worker.js` cache 更新至 `catstaste-order-v21-1-notes`，避免手機繼續食舊版 cache
...
- 改價 / 改九折 / 改滿額設定時加入確認提示
- `service-worker.js` cache 更新至 `catstaste-order-v21-1-notes`
